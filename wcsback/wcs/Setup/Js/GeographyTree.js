...
function configMenu(curNode){         
    ...
    mMain.mCopy.disabled = f_isRoot(curNode) ||  f_isMore(curNode) || !f_isLeaf(curNode);
    mMain.mCut.disabled = f_isRoot(curNode)  ||  f_isMore(curNode) || !f_isLeaf(curNode);
...
function refreshParentNode(mode,type){   
    ...
    else if(mode == "EDIT"){var geoId = f_getNodeID(oCurrNode);WsSetup.GetGeographyName(geoId,callBackRefreshNodeText,f_wserror);}     
}
...
function refreshNode(oNode){
    ...
    oNode.setAttribute("treeNodeSrc","GeographyGetChildNodes.aspx?parentNodeID=" + f_getNodeID(oNode));oNode.databind();
}
//---------------------------------------------------------------------------------------------------
function deleteMenu(){if(!CheckDelete()){return;}else{removeSelectedNode();} }
function removeSelectedNode(){var oCurrNode = f_getSelectedNode(TVOrg);var geoId = f_getNodeID(oCurrNode);WsSetup.DeleteGeography(geoId,refreshRemoveNodes,f_wserror);}
